...
def main() -> None:
    ...
    require('terraz-assets" content="20260724-9' in html, "Homepage assets version was not bumped")
    require(parser.logo_object.get("type") == "image/svg+xml", "Logo must be embedded as an SVG document")
    require("terraz-logo.svg?v=20260724-9" in (parser.logo_object.get("data") or ""), "Logo object source is stale")
    require("hero__logo--base" not in html and "hero__logo--trace" not in html, "Fake two-layer logo reveal must not return")
    require("logo-drawing.css?v=20260724-9" in html, "True logo drawing stylesheet is not linked")
    require("logo-animation.js?v=20260724-9" in html, "True logo drawing script is not linked")
    ...
    require("!important" not in logo_js, "Logo animation must not freeze itself with important declarations")
    ...
    require("motion-preferences.css?v=20260724-9" in html, "Motion compatibility stylesheet is not current")
