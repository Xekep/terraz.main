...
def main() -> None:
    ...
    require("REVEAL_STROKE_WIDTH = 8" in logo_js, "Exact mask brush width changed")
    require("REVEAL_PATHS" not in logo_js, "Approximate replacement paths must not return")
    require('finalPath.cloneNode(false)' in logo_js, "Reveal brush must clone the exact original path geometry")
    require('revealPath.classList.add("logo-reveal-stroke")' in logo_js, "Exact mask brush class is missing")
    require('finalGroup.classList.add("logo-final-shape")' in logo_js, "Original logo must be wrapped in a root-coordinate group")
    require('finalGroup.setAttribute("mask"' in logo_js, "Reveal mask must be applied to the outer logo group")
    require('finalPath.setAttribute("mask"' not in logo_js, "Reveal mask must not use the transformed original path coordinate space")
    require('finalPath.removeAttribute("mask")' in logo_js, "Stale path masks must be removed")
    require('drawing.id = "terraz-logo-reveal"' in logo_js, "Exact mask animation must be identifiable")
    require('direction: "normal"' in logo_js and "iterations: 1" in logo_js, "Mask brush must draw once forward")
    require('path.style.strokeDashoffset = "0"' in logo_js, "Completed mask brush must remain committed")
    require('logoHost.dataset.strokeCount = "1"' in logo_js, "Logo must use one exact reveal brush")
