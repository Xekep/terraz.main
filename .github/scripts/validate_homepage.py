...
def main() -> None:
    ...
    require('terraz-assets" content="20260724-8' in html, "Homepage assets version was not bumped")
    require(parser.logo_object.get("type") == "image/svg+xml", "Logo must be embedded as an SVG document")
    require("terraz-logo.svg?v=20260724-8" in (parser.logo_object.get("data") or ""), "Logo object source is stale")
    require("hero__logo--base" not in html and "hero__logo--trace" not in html, "Fake two-layer logo reveal must not return")
    require("logo-drawing.css?v=20260724-8" in html, "True logo drawing stylesheet is not linked")
    require("logo-animation.js?v=20260724-8" in html, "True logo drawing script is not linked")
    require("getTotalLength" in logo_js, "Logo path length must be measured from the real SVG path")
    require("stroke-dashoffset" in logo_js and "terraz-logo-write" in logo_js, "True SVG stroke drawing animation is missing")
    ...
    require("motion-preferences.css?v=20260724-8" in html, "Motion compatibility stylesheet is not current")
