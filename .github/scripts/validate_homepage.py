...
def main() -> None:
    ...
    require('terraz-assets" content="20260725-6' in html, "Homepage assets version was not bumped")
    require(parser.logo_host.get("role") == "img", "Logo host must expose image semantics")
    require("terraz-logo.svg?v=20260725-6" in (parser.logo_host.get("data-logo-src") or ""), "Inline logo source is stale")
    require("<object" not in html, "Opaque object-based logo canvas must not return")
    require("logo-drawing.css?v=20260725-6" in html, "Logo drawing stylesheet is not linked")
    require("logo-animation.js?v=20260725-6" in html, "Logo drawing script is not linked")
    ...
    require('path.style.strokeDashoffset = "0"' in logo_js, "Completed mask strokes must remain committed")
    ...
    require("motion-preferences.css?v=20260725-6" in html, "Motion compatibility stylesheet is not current")
